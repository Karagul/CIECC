...
data = read.csv("C:/Users/Administrator/Desktop/宏观组/最终消费支出预测/最终消费.csv")
head(data)

#取对数
tsdata = ts(data$实际最终消费支出1978.100,start=1978)
plot(tsdata)
tsdata = log(tsdata)
plot(tsdata)


p1 = ggplot(data,aes(年份,实际最终消费支出1978.100,colour = I("steelblue")))+
  geom_line(size = 1)+geom_point(size=3)+xlab("时间")+ylab("实际最终消费支出(亿元)")
p2 = ggplot(data,aes(年份,log(实际最终消费支出1978.100),colour = I("steelblue")))+
  geom_line(size = 1)+geom_point(size=3)+xlab("时间")+ylab("对数实际最终消费支出(亿元)")
grid.newpage()
...
arima1 = Arima(m1,order=c(3,1,1),method="ML");arima1
...
plot(tsdata,type = "b")
legend("bottomright",legend = c("真实值","拟合值"),col = c("black","red"),lty=c(1,1))
points(arima1$fitted,type = "b",col = "red")
...
##模型比较
# 对数预测
pre1 = forecast(arima1,h=4)$mean
pre2 = forecast(X2,h=4)$mean

plot(m2,ylim=c(10.70,11.10))
points(pre1,type="b",col="blue")
points(pre2,type="b",col="green")
...
y=data[(dim(data)[1]-length(pre1)+1):dim(data)[1],c("年份","实际最终消费支出1978.100")]
y$arima模型预测值 = pre1
y$双参数指数平滑预测值 = pre2
y
...
ggplot(y1, aes(x=年份, y=value, group=variable,color=variable)) + 
  geom_line(size = 1) +geom_point(size=2)+ labs(x='年份',y='实际最终消费支出') +
...
plot(forecast(arima_all,h=8),main="",xlab="年份",ylab="对数实际最终消费支出")
pre = forecast(arima_all,h=8)
pre_value = ts(c(pre$x,pre$mean),start = 1978)
pre_value = exp(pre_value)
pre_lower = exp(pre$lower[,2])
pre_upper = exp(pre$upper[,2])

plot(pre_value,lwd=2,xlab="年份",ylab="实际最终消费支出")
